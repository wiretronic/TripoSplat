...
def preprocess_image(image, erode_radius: int = 1) -> Image.Image:
...
class TripoSplatPipeline:
    def __init__(self, ckpt_path: str, decoder_path: str, dinov3_path: str,
                 flux2_vae_encoder_path: str, device: str = "cuda"):
    ...
    def preprocess_image(self, image, erode_radius: int = 1) -> Image.Image:
        return preprocess_image(image, erode_radius=erode_radius)
    ...
    def run(self, image, seed: int = 42, steps: int = 20, guidance_scale: float = 3.0,
        ...
        Args:
            image: Input image. Accepts a file path / PIL.Image / torch.Tensor
                ...
                channel as the 4th channel). Images with an RGBA alpha channel
                use the provided matte; images without alpha are padded on a
                black background.
            ...
            erode_radius: Pixel radius used to erode the alpha matte, to avoid
                segmentation-border bleed before compositing on black. `0`
                disables; `1` is a 3×3 minimum filter.
                Recommend: 1.
